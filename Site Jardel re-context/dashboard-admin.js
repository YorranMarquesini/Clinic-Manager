...
// -------------------- Garantir IDs em funcionários (compatibilidade) --------------------
let funcionarios = JSON.parse(localStorage.getItem("funcionarios")) || [];
(function ensureFuncionariosIds(){
    let changed = false;
    ...
    if (changed) localStorage.setItem("funcionarios", JSON.stringify(funcionarios));
})();
...
// funcionarios já carregado e normalizado acima
...
function adicionarFuncionario() {
    ...
    funcionarios.push({ id, nome, username, password: senha, tipo: tipoFinal, area });
...
function removerFuncionario(index) {
    if (confirm("Deseja realmente remover este funcionário?")) {
        ...
        // 1. Remover pacientes cadastrados por ele (compatível com cadastradoPorId ou cadastradoPor)
        let pacientes = JSON.parse(localStorage.getItem("pacientes")) || [];
        pacientes = pacientes.filter(p => {
            if (p.cadastradoPorId) return p.cadastradoPorId !== funcionario.id;
            return p.area !== funcionario.area || p.cadastradoPor !== funcionario.nome;
        });
        localStorage.setItem("pacientes", JSON.stringify(pacientes));

        // 2. Remover agendamentos desse médico (compatível com medicoId ou medico nome)
        let agendamentos = JSON.parse(localStorage.getItem("agendamentos")) || [];
        agendamentos = agendamentos.filter(a => {
            if (a.medicoId) return a.medicoId !== funcionario.id;
            return a.medico !== funcionario.nome;
        });
                ...
                if ((i === 0 && j < primeiroDia.getDay()) || dia > ultimoDia.getDate()) {
                    ...
                    if (agsDoDia.length > 0) {
                        ...
                        agsDoDia.forEach((a, idx) => {
                            ...
                            detalhes += `<li style="background-color:${cor}">${horario} - ${a.paciente} (Dr(a). ${medicoNome} - ${a.area})</li>`;
                        });
                        detalhes += "</ul></div>";
