...
// -------------------- Inicialização: garantir IDs e normalizar dados --------------------
(function ensureDataIds(){
    ...
    for (let i = 0; i < funcionarios.length; i++) {
        if (!funcionarios[i].id) {
            funcionarios[i].id = Date.now().toString() + i;
            changed = true;
        }
    }
    for (let i = 0; i < pacientes.length; i++) {
        if (!pacientes[i].id) {
            pacientes[i].id = Date.now().toString() + (1000 + i);
            changed = true;
        }
    }
    if (changed) {
    ...
    // Normalizar agendamentos (adicionar pacienteId / medicoId quando possível)
    let agendamentos = JSON.parse(localStorage.getItem("agendamentos")) || [];
    let agChanged = false;
    agendamentos = agendamentos.map(a => {
        if (a.paciente && !a.pacienteId) {
            const p = pacientes.find(pp => pp.nome === a.paciente || pp.login === a.paciente);
            if (p) { a.pacienteId = p.id; agChanged = true; }
        }
        if (a.medico && !a.medicoId) {
            const m = funcionarios.find(ff => ff.nome === a.medico || ff.username === a.medico);
            if (m) { a.medicoId = m.id; agChanged = true; }
        }
        return a;
    });
    if (agChanged) localStorage.setItem("agendamentos", JSON.stringify(agendamentos));

    // Normalizar inbox (adicionar destinatarioId / remetenteId quando possível)
    let inbox = JSON.parse(localStorage.getItem("inboxMensagens")) || [];
    let inboxChanged = false;
    inbox = inbox.map(msg => {
        if (msg.destinatario && !msg.destinatarioId) {
            const u = funcionarios.find(f => f.nome === msg.destinatario || f.username === msg.destinatario) || pacientes.find(p => p.nome === msg.destinatario || p.login === msg.destinatario);
            if (u) { msg.destinatarioId = u.id; inboxChanged = true; }
        }
        if (msg.remetente && !msg.remetenteId) {
            const u = funcionarios.find(f => f.nome === msg.remetente || f.username === msg.remetente) || pacientes.find(p => p.nome === msg.remetente || p.login === msg.remetente);
            if (u) { msg.remetenteId = u.id; inboxChanged = true; }
        }
        return msg;
    });
    if (inboxChanged) localStorage.setItem("inboxMensagens", JSON.stringify(inbox));
})();
...
// -------------------- Helpers para resolver nomes/ids --------------------
...
function atualizarIndicadorInbox() {
    ...
    const naoLidas = inbox.filter(msg => msg.destinatarioId === usuario.id);
...
function notificarConsulta(destinatarioId, consulta, marcada) {
    const remetenteId = "system";
    const texto = marcada
        ? `Consulta marcada: ${consulta.data} com ${consulta.medico}`
        : `Consulta desmarcada: ${consulta.data} com ${consulta.medico}`;
    adicionarMensagem(destinatarioId, remetenteId, "consulta", texto);
}
            ...
            <div class="card">
                ...
                <ul>
                    ${pacientes.map((p, index) => {
                        ...
                        return `
                            <li>
                                <span style="cursor:pointer; color:blue;" onclick="verPacienteDetalhes('${p.id}')">
                                    ${p.nome} - ${idade} anos
                                </span>
...
function verPacienteDetalhes(id) {
    const sub = document.getElementById("subConteudo");
    let pacientes = JSON.parse(localStorage.getItem("pacientes")) || [];
    const usuario = JSON.parse(localStorage.getItem("usuarioLogado")) || {};
    const paciente = pacientes.find(p => p.id === id && p.area === usuario.area);
    if (!paciente) {
        sub.innerHTML = `<div class="card"><p>Paciente não encontrado.</p></div>`;
        return;
    }
    ...
    let passados = agendamentos.filter(a => 
        (a.pacienteId ? a.pacienteId === paciente.id : a.paciente === paciente.nome) && 
        a.area === usuario.area && 
        a.concluida === true
    );
    ...
    document.getElementById("formDetalhesPaciente").addEventListener("submit", function(e){
        e.preventDefault();
        let pacientesAll = JSON.parse(localStorage.getItem("pacientes")) || [];
        const realIndex = pacientesAll.findIndex(p => p.id === paciente.id && p.area === usuario.area);
        if (realIndex >= 0) {
            pacientesAll[realIndex].celular = document.getElementById("celularPaciente").value.trim();
            pacientesAll[realIndex].email = document.getElementById("emailPaciente").value.trim();
            localStorage.setItem("pacientes", JSON.stringify(pacientesAll));
    ...
    if (passados.length > 0) {
        ...
        salvarNotaBtn.addEventListener("click", function(e) {
            ...
            // achar índice real no storage (usar pacienteId/medicoId quando disponível)
            const indexReal = agendamentos.findIndex(a => 
                (a.pacienteId ? a.pacienteId === agendamentoEscolhido.pacienteId : a.paciente === agendamentoEscolhido.paciente) &&
                a.data === agendamentoEscolhido.data
            );
            if (indexReal >= 0) {
...
function removerPaciente(id) {
    let pacientes = JSON.parse(localStorage.getItem("pacientes")) || [];
    const usuario = JSON.parse(localStorage.getItem("usuarioLogado")) || {};
    const pacienteRemovido = pacientes.find(p => p.id === id && p.area === usuario.area);

    if (!pacienteRemovido) return;

    if (confirm(`Deseja realmente remover o paciente "${pacienteRemovido.nome}"?`)) {
        pacientes = pacientes.filter(p => !(p.area === usuario.area && p.id === pacienteRemovido.id));
        localStorage.setItem("pacientes", JSON.stringify(pacientes));

        let agendamentos = JSON.parse(localStorage.getItem("agendamentos")) || [];
        agendamentos = agendamentos.filter(a => !( (a.pacienteId ? a.pacienteId === pacienteRemovido.id : a.paciente === pacienteRemovido.nome) && a.area === usuario.area ));
        localStorage.setItem("agendamentos", JSON.stringify(agendamentos));
            ...
            <div class="card">
                ...
                <ul>
                    ${agendamentos.map((a, idx) => {
                        ...
                        const medicoNome = a.medico || (a.medicoId ? (JSON.parse(localStorage.getItem("funcionarios"))||[]).find(f=>f.id===a.medicoId)?.nome : "—");

                        return `
                            <li>
                                ${horarioFormatado} - ${a.paciente} (Dr(a). ${medicoNome})
                                ${botoes}
                            </li>
                        `;
                    }).join("")}
...
function salvarNotaConsulta(index) {
    ...
    // Atualiza histórico do paciente (encontra por id quando possível)
    let pacientes = JSON.parse(localStorage.getItem("pacientes")) || [];
    const pacienteIndex = pacientes.findIndex(p => p.id === agendamento.pacienteId || p.nome === agendamento.paciente);
...
function cadastrarAgendamento() {
    ...
    let optionsPacientes = pacientesArea.map(p => `<option value="${p.id}">${p.nome}</option>`).join("");

    function gerarHorariosDisponiveis(dataSelecionada) {
        const agendamentos = JSON.parse(localStorage.getItem("agendamentos")) || [];
        const agendamentosMedico = agendamentos.filter(a => (a.medicoId ? a.medicoId === usuario.id : a.medico === usuario.nome));
    ...
    document.getElementById("formAgendamento").addEventListener("submit", function(e){
        e.preventDefault();
        const pacienteId = document.getElementById("pacienteAgendamento").value;
        const pacientesAll = JSON.parse(localStorage.getItem("pacientes")) || [];
        const pacienteObj = pacientesAll.find(p => p.id === pacienteId);
        ...
        // Notificação
        notificarConsulta(pacienteObj.id, { medico: usuario.nome, data: new Date(data).toLocaleString("pt-BR") }, true);
...
function removerAgendamento(index) {
    ...
    if(confirm("Deseja realmente remover este agendamento?")) {
        ...
        const pacientes = JSON.parse(localStorage.getItem("pacientes")) || [];
        const pacienteObj = pacientes.find(p => (agendamento.pacienteId ? p.id === agendamento.pacienteId : p.nome === agendamento.paciente) && p.area === agendamento.area);

        agendamentos.splice(index, 1);
        localStorage.setItem("agendamentos", JSON.stringify(agendamentos));

        if(pacienteObj) {
            notificarConsulta(pacienteObj.id, { medico: agendamento.medico, data: new Date(agendamento.data).toLocaleString("pt-BR") }, false);
        }
